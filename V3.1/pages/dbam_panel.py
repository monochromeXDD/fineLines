...
st.set_page_config(page_title="FineLines - ADMIN", page_icon="assets/alba_logo.png",initial_sidebar_state="collapsed")
...
if st.button("Upload Document"):
    if not all([doc_id, custom_title, uploaded_file]):
        ...
        with st.spinner("Processing document..."):
            try:
                # 🟢 FIX: Read the file once and store the bytes in memory
                pdf_bytes = uploaded_file.read()
                
                # Process PDF using the stored bytes
                documents = chroma_manager.process_pdf(pdf_bytes, uploaded_file.name)
